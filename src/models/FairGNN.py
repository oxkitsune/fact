...
class BaseAC(nn.Module):
    def __init__(self, feature_dim, transformed_feature_dim, emb_dim, args):
        ...
        AC_params = (
            ...
            + list(self.fcdecoder.parameters())
            + list(self.hgnn_ac.parameters())
        )
        self.optimizer_AC = torch.optim.Adam(
...
class FairSelectAC(nn.Module):
    ...
    def fair_select(self, adj, feature_with_sens):
        sens = (
            feature_with_sens[:, -1] + 1
        )  # covert 0 to 1, 1 to 2. in case adj is 0 which cause wrong counter.
        ...
        for idx, row in enumerate(adj):
            ...
            sens_counter.remove(
                sens_counter[0]
            )  # ignore 0, which means the number of no edges nodes pairs
            ...
            remove_number = [
                max(counter - least_num_sens_class, 0) for counter in sens_counter
            ]  # number of edges per class that need to remove to keep fair
            for i, number in enumerate(remove_number):
                if number > 0:
                    sen_class = i + 1
                    sens_idx = np.where(sen_row.cpu() == sen_class)[0]
                    drop_idx = torch.tensor(
                        random.sample(list(sens_idx), number)
                    ).long()
                    adj[idx][drop_idx] = 0
        return adj
