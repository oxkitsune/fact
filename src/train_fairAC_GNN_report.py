...
def parser_args():
    ...
    parser.add_argument(
        "--lambda1", type=float, default=1.0, help="The hyperparameter of loss Lc"
    )
    parser.add_argument(
        "--lambda2",
        type=float,
    ...
    parser.add_argument(
        "--attn-drop", type=float, default=0.0, help="attention dropout"
    )
    parser.add_argument(
        "--negative-slope",
        type=float,
    ...
    parser.add_argument(
        "--attn_vec_dim", type=int, default=128, help="attention vector dim"
    )
    parser.add_argument(
        "--num_heads", type=int, default=1, help="the number of attention heads"
    )
    parser.add_argument(
        "--feat_drop_rate", type=float, default=0.3, help="feature dropout rate"
    )
    parser.add_argument(
        "--num_sen_class", type=int, default=1, help="number of sensitive classes"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--load", type=bool, default=False, help="load AC model, use with AC_model_path"
    )
    parser.add_argument(
        "--AC_model_path", type=str, default="./AC_model", help="AC_model_path"
    )
    parser.add_argument(
        "--GNN_model_path", type=str, default="./GNN_model", help="GNN_model_path"
    )
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if epoch < args.epochs and not args.load:
            ...
            feat_keep_idx, feat_drop_idx = train_test_split(
                np.arange(ac_train_idx.shape[0]), test_size=args.feat_drop_rate
            )
            ...
            feature_src_re2, features_hat, transformed_feature = ACmodel(
                ...
                embedding[ac_train_idx][feat_keep_idx],
            ...
            Csen_loss = criterion(
                sens_prediction_detach, sens_train[feat_keep_idx].unsqueeze(1).float()
            )
            ...
            if epoch > 1000 and epoch % 200 == 0 or epoch == args.epochs - 1:
                with torch.no_grad():
                    ...
                    for i, sub_node in enumerate(sub_nodes):
                        ...
                        features_embedding[
                            sub_node[feat_drop_idx_sub]
                        ] = feature_src_AC[feat_drop_idx_sub]
                        features_embedding[
                ...
                for sub_epoch in range(1000):
                    ...
                    Cy_loss = GNNmodel_inside.criterion(
                        y[y_idx], labels[idx_train].unsqueeze(1).float()
                    )
                    ...
                    if sub_epoch % 100 == 0:
                        print(
                            "Epoch: {:04d}, sub_epoch: {:04d}, loss_ac: {:.4f}, loss_reconstruction: {:.4f}, Csen_loss: {:.4}, Csen_adv_loss: {:.4f}, Cy_loss: {:.4f}".format(
                                epoch,
                                sub_epoch,
                                loss_ac.item(),
                                loss_reconstruction.item(),
                    ...
                    if acc_test > args.acc and roc_test > args.roc:
                        if best_fair > parity + equality:
                            ...
                            torch.save(
                                GNNmodel_inside,
                                save_path
                                + "testGNNinside_epoch{:04d}_acc{:.4f}_roc{:.4f}_par{:.4f}_eq_{:.4f}".format(
                                    epoch, acc_test.item(), roc_test, parity, equality
                                ),
                            )
                            torch.save(
                                ACmodel,
                                save_path
                                + "testACmodelinside_epoch{:04d}_acc{:.4f}_roc{:.4f}_par{:.4f}_eq_{:.4f}".format(
                                    epoch, acc_test.item(), roc_test, parity, equality
                                ),
                            )

                        print("=================================")
                        log = "Epoch: {:04d}, loss_ac: {:.4f}, loss_reconstruction: {:.4f}, Csen_loss: {:.4}, Csen_adv_loss: {:.4f}, cls: {:.4f}".format(
                            epoch,
                            loss_ac.item(),
                            loss_reconstruction.item(),
                            Csen_loss.item(),
                        ...
                        log = "Test: accuracy: {:.4f} roc: {:.4f} parity: {:.4f} equality: {:.4f}\n".format(
                            acc_test.item(), roc_test, parity, equality
                        )
                        with open(log_file, "a") as f:
                            f.write(log)
    ...
    if len(best_result) > 0:
        log = "Test: accuracy: {:.4f}, roc: {:.4f}, parity: {:.4f}, equality: {:.4f}".format(
            best_result["acc"],
            best_result["roc"],
            best_result["parity"],
            best_result["equality"],
        )
        with open(test_log_file, "a") as f:
            f.write(log)
        print(
